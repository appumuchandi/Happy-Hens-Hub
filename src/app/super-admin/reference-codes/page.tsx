...
import { getReferenceCodes } from './actions';
import { ReferenceCodeClient } from '@/components/admin/reference-code-client';
...
export default async function ReferenceCodeManagementPage() {
  let codes: any[] = [];
  let error: string | null = null;

  try {
    codes = await getReferenceCodes();
  } catch (e: any) {
    ...
    <div className="flex flex-col min-h-screen bg-slate-50/50">
      <main className="flex-grow py-8">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="mb-8">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h1 className="text-3xl font-black text-slate-900 tracking-tight">Partner Codes</h1>
                  <p className="text-slate-500 font-medium text-sm mt-1">
                    Manage referral and partner reference codes
                  </p>
                </div>
              </div>
            </div>
          ...
          <ReferenceCodeClient initialCodes={codes} />
        </div>
      </main>
    </div>
  );
